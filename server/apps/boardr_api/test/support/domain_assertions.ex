defmodule BoardrApi.DomainAssertions do
  ...
  def assert_user_resource(body, %User{} = user) do
    assert_api_map(body)
    ...
    |> assert_hal_links(fn links ->
      links
      |> assert_hal_curies()
      |> assert_hal_link("self", test_api_url("/users/#{user.id}"))
    end)

    # Properties
    |> assert_key("createdAt", DateTime.to_iso8601(user.created_at))
    |> assert_key("name", user.name)
    |> assert_key("updatedAt", DateTime.to_iso8601(user.updated_at))
  end
end
